...
import { Package, Clock, MapPin, CheckCircle2, AlertCircle } from "lucide-react";
...
const MyFoodRequests = () => {
    ...
    const { data: results, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await axios.get(`http://localhost:5000/api/requests/my-requests/${user?.email}`);
            return res.data;
        },
        enabled: !!user?.email
    });
                        ...
                        >
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <div className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest flex items-center ${
                                    req.requestStatus === 'Delivered' ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'
                                }`}>
                                    {req.requestStatus === 'Delivered' ? <CheckCircle2 size={12} className="mr-1.5" /> : <Clock size={12} className="mr-1.5" />}
                                    {req.requestStatus}
                                </div>
                            </div>
